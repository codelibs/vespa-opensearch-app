...
public class SettingsAction extends HttpAction {

    public SettingsAction(final RestApiProxyHandler handler) {
        super(handler);
    }
    ...
    public boolean isTarget(final Method method, final String[] paths) {
        // GET /<index>/_settings
        // PUT /<index>/_settings
        if (paths.length >= 3 && !paths[1].startsWith("_") && "_settings".equals(paths[2])) {
    ...
    public HttpResponse execute(final HttpRequest httpRequest) {
        ...
        try {
            if (method == Method.GET) {
                return handleGetSettings(httpRequest, indexName, client);
            } else if (method == Method.PUT) {
                return handleUpdateSettings(httpRequest, indexName, client);
            } else {
                final Map<String, Object> error = new HashMap<>();
    ...
    private HttpResponse handleGetSettings(final HttpRequest httpRequest, final String indexName, final VespaClient client) {
        final Map<String, Object> result = client.getSettings(indexName);
        return createResponse(httpRequest, 200, result);
    }

    private HttpResponse handleUpdateSettings(final HttpRequest httpRequest, final String indexName, final VespaClient client) {
        ...
        final Map<String, Object> result = client.updateSettings(indexName, requestBody);
        return createResponse(httpRequest, 200, result);
    }
